...
#include "rebind.h"
 ...
 typedef struct user_conn_s {
     ...
     int                 tracked_pkt_cnt;
     int                 is_reconnecting; // : add to indicate whether the connection is in the process of reconnecting
 } user_conn_t;
 ...
 char g_file_path[128] = ""; // : 测试长连接传输文件地址
 char g_test_type[20] = "normal-test"; // :测试类型
 int g_reset = 0; // :迁移后重置user_conn参数
 ...
 */
         ...
         if (res < 0) {
             ...
             if (errno == EAGAIN) {
                 ...
                 if (connection_migration_enabled && retry_cnt < g_max_retry) {
                    ...
                    user_conn->is_reconnecting = 1; //  : reconnect
                    retry_cnt++;
                    continue;
                 } else {
 ...
 {
     int ip_type = (g_ipv6 ? AF_INET6 : AF_INET);
     user_conn->fd = xqc_client_create_socket(ip_type,  // : change NULL to enp6s19
 ...
 xqc_client_h3_conn_close_notify(xqc_h3_conn_t *conn, const xqc_cid_t *cid, void *user_data)
 {
     ...
     //     on_enp6s19_ip_reconnect(user_conn); // : 如果是重连，重新连接
 ...
 void xqc_conn_client_resend_immediately(xqc_connection_t *conn){
    xqc_list_head_t *head = &conn->conn_send_queue->sndq_unacked_packets[2]; // : 只重传应用数据，也就是XQC_PNS_APP_DATA
    int congest = 1;
 ...
 int on_ip_change(void* arg) {
     user_conn_t *user_conn = (user_conn_t *)arg;
     printf("[INFO] IP changed, will update socket! old fd: %d\n", user_conn->fd);
    ...
    while (retries > 0) {
        ...
        if (ret == 0) {
            // Socket创建成功，跳出循环
            printf("[INFO] Successfully created new socket for migration. New fd: %d\n", user_conn->fd);
            // 在成功创建新socket后，主动发送一个PING帧
            xqc_conn_send_ping(user_conn->ctx->engine, &user_conn->cid, NULL);
            return ret;
        } else {
            printf("[ERROR] Failed to create socket, retrying... (%d retries left)\n", retries - 1);
            usleep(delay_ms * 1000); // 转换成微秒
            delay_ms *= 2; // 指数级回退，下次延迟加倍
            retries--;
        }
    }

    printf("[ERROR] Failed to create socket after multiple retries. Migration failed.\n");
     return ret;
 }
...
//  }
 ...
 int main(int argc, char *argv[]) {
     ...
     if(connection_migration_enabled)
         register_netlink_event(eb, on_ip_change, user_conn, g_multi_interface[0]); //  :register ip_rebind
