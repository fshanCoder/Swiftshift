 ...
 */
 ...
 #include <sys/resource.h> //for ffmpeg CPU test
 ...
 typedef struct user_stream_s {
     ...
     xqc_usec_t               start_time; // : 实时流开始时间
     struct event            *ev_frame_timer; // : 定时器，用于实时流定时发送
    uint32_t                 frame_counter; // : 帧计数器
    int                       closed; // : 流是否发送完成
    int                       is_live_ingest; // : 是否为客户端实时推流
    } user_stream_t;
 ...
 int g_enable_fec = 0;
 int g_debug = 0; // : for debug print
 int g_delay_challenge = 0; // : 置为1，则延迟认证PATH_CHALLENGE，相当于0-RTT连接迁移
 int g_immediate_resend = 0; // : 迁移后立即重传未确认包
...
// :实时数据流测试
int g_frame_rate_fps = 100; // 假设每秒120帧
size_t g_frame_size_bytes = 1000; // 每帧大小，假设为100KB
uint64_t g_next_frame_send_ts = 0;
int g_real_time = 0; // :实时传输标志

 char g_send_file_path[256] = {0}; //GET请求发送的文件
 char g_url[256];
 ...
 #pragma pack(push, 1)
typedef struct {
    uint32_t frame_id;    // 帧号（递增）
    uint64_t send_ts;     // 发送时间戳(us)
} frame_hdr_t;
#pragma pack(pop)



static FILE *g_cwnd_fp = NULL;

static void cwnd_log_close(void) {
    if (g_cwnd_fp) { fclose(g_cwnd_fp); g_cwnd_fp = NULL; }
}

static inline FILE* cwnd_log_file(void) {
    if (!g_cwnd_fp) {
        g_cwnd_fp = fopen("cwnd.log", "a");   // 追加写
        if (g_cwnd_fp) {
            setvbuf(g_cwnd_fp, NULL, _IOLBF, 0);  // 行缓冲：每行自动flush
            atexit(cwnd_log_close);               // 进程退出时关闭
        }
    }
    return g_cwnd_fp;
}

// 替换/新增: 从 stdin 读取数据并发送, 适用于管道输入
/* 从 stdin/缓冲区持续送数据的定时器回调 */
static void server_send_from_stdin_cb(evutil_socket_t fd, short events, void *arg) {
    ...
    xqc_h3_conn_t *h3c = us->h3_request->h3_stream->h3c;
    if (h3c && h3c->conn) {
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list) {
        ...
        if (send_ctl && send_ctl->ctl_cong_callback && send_ctl->ctl_cong) {
            uint64_t cwnd = send_ctl->ctl_cong_callback->xqc_cong_ctl_get_cwnd(send_ctl->ctl_cong);
            FILE *fp = cwnd_log_file();
        if (fp) {
            fprintf(fp,
                "[CWND] ts=%" PRIu64 "us path=%ld cwnd=%" PRIu64 " bytes\n",
                xqc_now(), (long)path->path_id, cwnd);
            }
        }
        }
    }

    /* 1) 如果还有上次没发完的“待发送缓冲”，先尽量吐出去 */
    if (us->send_body && us->send_offset < us->send_body_len) {
        size_t pend = us->send_body_len - us->send_offset;
        ret = xqc_h3_request_send_body(us->h3_request,
                                       us->send_body + us->send_offset, pend, 0);
        if (ret > 0) {
            us->send_offset += ret;
        } else if (ret == -XQC_EAGAIN) {
            /* 发送窗口满了，过会儿再来 */
            struct timeval tv = {0, 1000}; /* 1ms */
            evtimer_add(us->ev_frame_timer, &tv);
            return;
        } else if (ret < 0) {
            printf("[server] send_body error %zd\n", ret);
        }
        /* 仍可能没发完，等下一拍 */
        if (us->send_offset < us->send_body_len) {
            struct timeval tv = {0, 1000};
            evtimer_add(us->ev_frame_timer, &tv);
            return;
        }
        /* 全发完了，清空缓冲区占用（不 free 指针，复用同一块内存） */
        us->send_offset = us->send_body_len = 0;
    }

    /* 2) 读 stdin（非阻塞）；为了简单使用一块临时小缓冲把数据复制进 send_body 再发 */
    ...
    if (n < 0) {
        ...
        perror("[server] read(stdin) error");
    ...
    if (n == 0) {
        /* EOF：结束响应，发 FIN */
        printf("[server] EOF on stdin, send FIN\n");
        xqc_h3_request_finish(us->h3_request);
        // if (us->ev_frame_timer) {
        //     event_free(us->ev_frame_timer);
        //     us->ev_frame_timer = NULL;
        // }
        us->closed = 1;
        /* 打印统计信息 */
    char buf[512];
    xqc_h3_request_stats_print(us->h3_request, buf, sizeof(buf));
    printf("[server] Request stats: %s\n", buf);
    }

    /* 把 tmp 拷到 send_body，当作“待发送缓冲”使用，避免读了但发不出去造成丢数据 */
    if (us->send_body == NULL) {
        /* 首次分配一块常驻缓冲，复用即可 */
        us->send_body = malloc(1024 * 1024);
        us->send_body_max = 1024 * 1024;
        us->send_offset = us->send_body_len = 0;
    }
    if ((size_t)n > (us->send_body_max)) {
        /* 输入块异常大，截断保护（正常不会发生，因为 tmp=1500） */
        n = us->send_body_max;
    }
    memcpy(us->send_body, tmp, (size_t)n);
    us->send_offset = 0;
    us->send_body_len = (size_t)n;

    /* 先尝试立刻发送这批数据 */
    ret = xqc_h3_request_send_body(us->h3_request, us->send_body, us->send_body_len, 0);
    if (ret > 0) {
        us->send_offset = (size_t)ret;
    } else if (ret == -XQC_EAGAIN) {
        /* 发送窗口满，保持缓冲，下次再发 */
    } else if (ret < 0) {
        printf("[server] send_body error %zd\n", ret);
 ...
 // : 设置定时器
static void
send_frame_timer_callback(int fd, short what, void *arg)
{
    ...
    // 实验时长控制
    if (user_stream->start_time > 0 && now_us - user_stream->start_time >= 60000000) {
        printf("30-second streaming test finished. Sending FIN.\n");
        xqc_h3_request_finish(user_stream->h3_request);
        event_free(user_stream->ev_frame_timer);
        user_stream->ev_frame_timer = NULL;
        return;
    }

    // --- 构造帧数据 ---
    size_t payload_size = g_frame_size_bytes - sizeof(frame_hdr_t);
    if (payload_size < 0) {
        printf("Error: g_frame_size_bytes too small!\n");
        return;
    }

    frame_hdr_t hdr;
    hdr.frame_id = htonl(++user_stream->frame_counter); // 递增帧号
    hdr.send_ts  = htobe64(now_us);                     // 微秒时间戳

    // 分配 buffer: header + payload
    size_t total_size = sizeof(hdr) + payload_size;
    char *send_buf = malloc(total_size);
    memcpy(send_buf, &hdr, sizeof(hdr));
    memset(send_buf + sizeof(hdr), 'A', payload_size);  // payload 示例

    // 发送
    ssize_t ret = xqc_h3_request_send_body(user_stream->h3_request, send_buf, total_size, 0);
    // printf("[Server] FrameID=%u, TS=%" PRIu64 ", Size=%zu\n",
    //    ntohl(hdr.frame_id), now_us, total_size);
    if (ret < 0) {
        if (ret != -XQC_EAGAIN) {
    ...
    // --- 重启定时器 ---
    struct timeval tv;
    uint64_t frame_interval_us = 1000000 / g_frame_rate_fps;
    tv.tv_sec = frame_interval_us / 1000000;
    tv.tv_usec = frame_interval_us % 1000000;

    event_add(user_stream->ev_frame_timer, &tv);

    // printf("Sent frame %u at %lu us (size=%zu)\n",
    //        ntohl(hdr.frame_id), now_us, total_size);
}
 ...
 */
 ...
 xqc_server_request_send(xqc_h3_request_t *h3_request, user_stream_t *user_stream)
 {
    ...
    // 构建响应头
    // 状态码
    header[header_cnt++] = (xqc_http_header_t) {
    ...
    // 在实时流模式下，发送头部，但FIN标志为0
        ...
        printf("Real-time streaming headers sent. Stream ID: %lu\n", xqc_h3_stream_id(h3_request));
        
        // --- 关键修正点 ---
                // --- 关键修正点：启动定时器 ---
        user_stream->h3_request = h3_request;
        user_stream->start_time = xqc_now();
        user_stream->ev_frame_timer = evtimer_new(eb, server_send_from_stdin_cb, user_stream);
        // user_stream->ev_frame_timer = event_new(eb, -1, 0, send_frame_timer_callback, user_stream);
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = 1000; // 立即开始，或稍作延迟
        event_add(user_stream->ev_frame_timer, &tv);
        return 0;
    }
                               ...
                               void *user_data)
{
    ...
    /* 记录第一次触发的时间（你原来的统计保留） */
    static xqc_usec_t start_time = 0;
    if (start_time == 0) {
        start_time = xqc_now();
    }
    ...
    if ((flag & XQC_REQ_NOTIFY_READ_HEADER) || (flag & XQC_REQ_NOTIFY_READ_TRAILER)) {
        ...
        for (int i = 0; i < headers->count; i++) {
            if (headers->headers[i].name.iov_len == 7 &&
                memcmp(headers->headers[i].name.iov_base, ":method", 7) == 0) {
                req_method = (const char *)headers->headers[i].value.iov_base;
            }
            if (headers->headers[i].name.iov_len == 5 &&
                memcmp(headers->headers[i].name.iov_base, ":path", 5) == 0) {
                req_path = (const char *)headers->headers[i].value.iov_base;
            }
        };

    if (req_method && strcmp(req_method, "POST") == 0 && strcmp(req_path, "/live") == 0) { // 不限 path，任意 POST 都进入
        // 回 200 响应头（不结束），保持会话活着以持续接收 body
        xqc_http_header_t hdrs[] = {
            { .name = { .iov_base=":status",      .iov_len=7 }, .value = { .iov_base="200",        .iov_len=3  } },
            { .name = { .iov_base="content-type", .iov_len=12}, .value = { .iov_base="text/plain", .iov_len=10 } },
        };
        xqc_http_headers_t resp = { .headers = hdrs, .count = 2 };
        ssize_t ret = xqc_h3_request_send_headers(h3_request, &resp, 0);
        if (ret < 0) {
            fprintf(stderr, "[server] send_headers error %zd\n", ret);
            return 0;
        }

        // 确保 stdout 无缓冲（只需设置一次；放初始化也行）
        setvbuf(stdout, NULL, _IONBF, 0);

        user_stream->is_live_ingest = 1;
        user_stream->recv_body_len  = 0;
        //fprintf(stderr, "[server] enter live-ingest (POST %s)\n", path ? path : "/");
        return 0; // 等待 READ_BODY
    }
    if (user_stream->is_live_ingest && (flag & XQC_REQ_NOTIFY_READ_BODY)) {
    unsigned char fin = 0;
    char    buf[65536];
    ...
    do {
        n = xqc_h3_request_recv_body(h3_request, (unsigned char*)buf, cap, &fin);
        if (n > 0) {
            user_stream->recv_body_len += (size_t)n;

            size_t w = fwrite(buf, 1, (size_t)n, stdout);  // 写到 stdout，供 ffmpeg pipe:0
            if (w != (size_t)n) {
                fprintf(stderr, "[server] fwrite(stdout) short write %zu/%zd\n", w, n);
                // 可选：根据需要决定是否立即结束
            }
        } else if (n == -XQC_EAGAIN) {
            break; // 暂无更多数据；下次回调继续
        } else if (n < 0) {
            fprintf(stderr, "[server] recv_body error %zd\n", n);
            xqc_h3_request_finish(h3_request);
            user_stream->is_live_ingest = 0;
            return 0;
        } else { // n == 0
            break;
        }
    ...
    if (fin) {
        fflush(stdout); // 确保最后一块送出
        fprintf(stderr, "[server] live-ingest FIN (total=%zu bytes)\n", user_stream->recv_body_len);
        xqc_h3_request_finish(h3_request);
        user_stream->is_live_ingest = 0;
    }
    return 0;
        ...
        /* ========= 1. 实时流模式 (需要 -z 打开 g_real_time) ========= */
        if (g_real_time && req_method && strcmp(req_method, "GET") == 0) {
            xqc_http_header_t resp_header[] = {
                { .name = { .iov_base = ":status",      .iov_len = 7 },
                  .value= { .iov_base = "200",          .iov_len = 3 } },
                { .name = { .iov_base = "content-type", .iov_len = 12 },
                  .value= { .iov_base = "video/MP2T",   .iov_len = 10 } },
            };
            xqc_http_headers_t resp_headers;
            resp_headers.headers = resp_header;
            resp_headers.count   = sizeof(resp_header) / sizeof(resp_header[0]);

            ret = xqc_h3_request_send_headers(h3_request, &resp_headers, 0);
            if (ret < 0) {
                printf("[server] send_headers error %d\n", ret);
                return ret;
            }
            ...
            struct timeval tv = {0, 1000}; /* 1ms */
            user_stream->ev_frame_timer =
                evtimer_new(eb, server_send_from_stdin_cb, user_stream);
            evtimer_add(user_stream->ev_frame_timer, &tv);

            printf("[server] 启动实时流模式\n");
            return 0;
        }
        /* ========= 3. 默认逻辑 ========= */
        ret = xqc_server_request_send(h3_request, user_stream);
...
//          // 检查请求方法
//         for (int i = 0; i < headers->count; i++) {
//             if (headers->headers[i].name.iov_len == 7 && memcmp(headers->headers[i].name.iov_base, ":method", 7) == 0) {
//                 if (headers->headers[i].value.iov_len == 3 && memcmp(headers->headers[i].value.iov_base, "GET", 3) == 0) {
//                     // 如果是 GET 请求并且指定了发送文件
//                     if (1) {
//                         user_stream_t *user_stream = (user_stream_t *)user_data;

//                         FILE *fp = fopen(g_send_file_path, "rb");
//                         if (fp == NULL) {
...
//                         user_stream->send_body_len = file_size;
//                         user_stream->send_body = malloc(user_stream->send_body_len);
//                         if (user_stream->send_body == NULL) {
//                             printf("Error: Malloc failed for send_body\n");
//                             fclose(fp);
//                             return -1;
//                         }
//                         fread(user_stream->send_body, 1, user_stream->send_body_len, fp);
//                         fclose(fp);

//                         // 准备响应头
//                         xqc_http_header_t resp_header[] = {
//                             { .name = { .iov_base = ":status", .iov_len = 7 }, .value = { .iov_base = "200", .iov_len = 3 } },
//                             { .name = { .iov_base = "content-type", .iov_len = 12 }, .value = { .iov_base = "application/octet-stream", .iov_len = 24 } },
//                             { .name = { .iov_base = "content-length", .iov_len = 14 }, .value = { .iov_base = "...", .iov_len = 3 } }, // 这里需要动态设置长度
//                         };
//                         char content_len_str[32];
//                         snprintf(content_len_str, sizeof(content_len_str), "%ld", file_size);
//                         resp_header[2].value.iov_base = content_len_str;
//                         resp_header[2].value.iov_len = strlen(content_len_str);

//                         xqc_http_headers_t resp_headers = { .headers = resp_header, .count = 3 };

//                         // 发送响应头
//                         xqc_h3_request_send_headers(h3_request, &resp_headers, 0);
...
// }
 ...
 int main(int argc, char *argv[]) {
     ...
     int ch = 0;
     while ((ch = getopt_long(argc, argv, "a:p:dDefc:Cs:w:r:l:u:x:6bS:MR:o:EK:mLQ:U:yH2zh:", long_opts, NULL)) != -1) {
         switch (ch) {
         case 'z':
            printf("option real_time test on\n");
            g_real_time = 1;
            break;
         case 'h':
            printf("option send file path: %s\n", optarg);
            snprintf(g_send_file_path, sizeof(g_send_file_path), optarg);
            break;
         case '2':
         ...
         case 'd':
             ...
             g_debug = 1;
             break;
         // : for delay validate PATH_CHALLENGE and PATH_RESPONSE
         case 'D':
             printf("option  : delay validate PATH_RESPONSE\n");
             g_delay_challenge = 1;
             break;
         case 'H':
     ...
     if (g_test_case == 1 || g_test_case == 5 || g_test_case == 6 || g_sid_len != 0) {
         ...
         // : set delay_challenge flag for engine's config
