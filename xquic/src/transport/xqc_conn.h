...
struct xqc_connection_s {
    ...
    xqc_usec_t                      conn_hsk_recv_time;
        ...
        /* send error exponential backoff control (for transient network errors during migration)
     * - send_retry_backoff_until: timestamp (usec) until which sending should be suppressed
     * - send_retry_attempts: number of consecutive transient-send failures observed
     * - send_retry_base_us: base backoff in microseconds (client may set; 0 means default 100000)
     * - send_retry_max_us: maximum backoff in microseconds (client may set; 0 means default 2000000)
     */
    xqc_usec_t                      send_retry_backoff_until;
    uint32_t                        send_retry_attempts;
    uint32_t                        send_retry_base_us;
    uint32_t                        send_retry_max_us;
