...
xqc_timer_nat_rebinding_timeout(xqc_timer_type_t type, xqc_usec_t now, void *user_data)
{
    ...
    /* If no PATH_RESPONSE yet and we still have rebinding_addr, retry PATH_CHALLENGE up to 3 times */
    if (path->rebinding_addrlen != 0 && path->rebinding_check_response == 0) {
        if (path->rebinding_count < 3) {
            xqc_log(conn->log, XQC_LOG_INFO, "|REBINDING|retry PATH_CHALLENGE|path:%ui|retry:%d|addr:%s|",
                    path->path_id, path->rebinding_count, xqc_path_addr_str(path));
            if (xqc_conn_send_path_challenge(conn, path) == XQC_OK) {
                path->rebinding_count++;
                /* re-arm NAT_REBINDING timer with PTO floor */
                ...
                xqc_timer_set(&path->path_send_ctl->path_timer_manager,
                              XQC_TIMER_NAT_REBINDING, now, 3 * pto);
                return;
            }
        }
    }

    /* give up: clear rebinding state and close the path */
    path->rebinding_addrlen = 0;
    path->rebinding_check_response = 0;
    xqc_set_path_state(path, XQC_PATH_STATE_CLOSED);// :如果超时，将路径置为关闭状态
}
