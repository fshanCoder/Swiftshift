...
xqc_process_path_response_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    /* quiet: use DEBUG log instead of stdout for PATH_RESPONSE */
    if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
        const char *reb_addr = (path->rebinding_addrlen > 0)
            ? xqc_peer_addr_str(conn->engine, (struct sockaddr*)path->rebinding_addr, path->rebinding_addrlen)
            : "-";
        printf("Recv PATH_RESPONSE | path:%lu | path_addr:%s | rebinding_addr:%s\n",
               path->path_id, xqc_path_addr_str(path), reb_addr);
        xqc_log(conn->log, XQC_LOG_DEBUG,
                "|REBINDING|Recv PATH_RESPONSE|path:%ui|path_addr:%s|rebinding_addr:%s|",
                path->path_id, xqc_path_addr_str(path), reb_addr);
    }
