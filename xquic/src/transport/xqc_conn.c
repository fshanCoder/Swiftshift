...
xqc_send(xqc_connection_t *conn, xqc_path_ctx_t *path, unsigned char *data, unsigned int len)
{
    ...
    /* If connection is in backoff window due to transient send errors, skip sending and retry later. */
    xqc_usec_t now_check = xqc_monotonic_timestamp();
    if (conn->send_retry_backoff_until) {
        if (conn->send_retry_backoff_until > now_check) {
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    "|xqc_send|conn:%p|path:%ui|backoff until:%llu us, retry later|",
                    conn, path ? path->path_id : 0,
                    (unsigned long long)conn->send_retry_backoff_until);
            return -XQC_EAGAIN;
        }

        /* backoff window elapsed, attempt to send again */
        conn->send_retry_backoff_until = 0;
    }
    if (conn->pkt_filter_cb) {
        ...
        if (sent != len) {
            ...
            /* if callback return XQC_SOCKET_ERROR, treat ENETUNREACH (transient during migration)
             * with exponential backoff; other errors remain treated as socket errors. */
            if (sent == XQC_SOCKET_ERROR) {
                int serr = errno;
                if (serr == ENETUNREACH) {
                    xqc_usec_t now2 = xqc_monotonic_timestamp();
                    uint32_t base = conn->send_retry_base_us ? conn->send_retry_base_us : 100000; /* 100ms */
                    uint32_t max = conn->send_retry_max_us ? conn->send_retry_max_us : 2000000; /* 2s */
                    uint32_t attempts = conn->send_retry_attempts;
                    uint64_t delay = (uint64_t)base << attempts;
                    if (delay > max) delay = max;
                    conn->send_retry_attempts = attempts + 1;
                    conn->send_retry_backoff_until = now2 + delay;
                    xqc_log(conn->log, XQC_LOG_WARN,
                            "|write_socket_ex transient ENETUNREACH|conn:%p|path:%ui|attempt:%u|backoff_us:%llu|",
                            conn, path->path_id, conn->send_retry_attempts, (unsigned long long)delay);
                    return -XQC_EAGAIN;
                }

                /* Non-transient socket error: mark path and let higher-level decide closure. */
                path->path_flag |= XQC_PATH_FLAG_SOCKET_ERROR;
                if (xqc_conn_should_close(conn, path)) {
        ...
        if (sent != len) {
            ...
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|write_socket error|conn:%p|size:%ud|sent:%z||errno:%d|%s|", conn, len, sent, errno, strerror(errno));

            /* if callback return XQC_SOCKET_ERROR, treat ENETUNREACH as transient and backoff */
            if (sent == XQC_SOCKET_ERROR) {
                int serr = errno;
                if (serr == ENETUNREACH) {
                    xqc_usec_t now3 = xqc_monotonic_timestamp();
                    uint32_t base = conn->send_retry_base_us ? conn->send_retry_base_us : 100000;
                    uint32_t max = conn->send_retry_max_us ? conn->send_retry_max_us : 2000000;
                    uint32_t attempts = conn->send_retry_attempts;
                    uint64_t delay = (uint64_t)base << attempts;
                    if (delay > max) delay = max;
                    conn->send_retry_attempts = attempts + 1;
                    conn->send_retry_backoff_until = now3 + delay;
                    xqc_log(conn->log, XQC_LOG_WARN,
                            "|write_socket transient ENETUNREACH|conn:%p|attempt:%u|backoff_us:%llu|",
                            conn, conn->send_retry_attempts, (unsigned long long)delay);
                    return -XQC_EAGAIN;
                }
    ...
    /* on successful send, clear transient send error state */
    if (sent > 0) {
        conn->send_retry_attempts = 0;
        conn->send_retry_backoff_until = 0;
    }
